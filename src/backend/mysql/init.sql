...
CREATE TABLE playlist (id BIGINT PRIMARY KEY AUTO_INCREMENT, nomPlaylist VARCHAR(30), thumbnail VARCHAR(256), createur VARCHAR(64));
CREATE TABLE possede (idUtilisateur BIGINT, idPlaylist BIGINT, PRIMARY KEY(idUtilisateur,idPlaylist));
CREATE TABLE video (id BIGINT PRIMARY KEY AUTO_INCREMENT, titre VARCHAR(300), lien VARCHAR(256), duree VARCHAR(10), site VARCHAR(50), thumbnail VARCHAR(256), vues BIGINT);
CREATE TABLE contient (idVideo BIGINT, idPlaylist BIGINT, PRIMARY KEY(idVideo, idPlaylist));
CREATE TABLE pub (id BIGINT PRIMARY KEY AUTO_INCREMENT, titre VARCHAR(50), clics BIGINT, moyenPaiement VARCHAR(50), extension VARCHAR(10), format VARCHAR(15), url VARCHAR(256), idUtilisateur BIGINT);
